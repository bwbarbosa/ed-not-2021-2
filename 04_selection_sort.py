...
# valor das posições sebsequentes. Faz a troca entre os dois valores, 
...
def selection_sort(lista):
    ...
    # Seleciona (isola) o elemento que será comparada
    for pos_sel in range(len(lista) - 1):
        passadas += 1

        pos_menor = pos_sel + 1

        # Rotina para encontrar o menos número no resto da lista       
        # Percurso da lista da posição i + 2 até o fim
        for j in range(pos_sel + 2, len(lista)):
            comps += 1

            # Se o elemento da posição atual (j) form menor
            # que o elemento na posição do menor (por_menor),
            # atualizamos pos_menor
            if lista[j] < lista[pos_menor]:
                pos_menor = j

        # Comparamos lista[sel] com lista[pos_menor] e, se segunda
